...
class ASTAR(object):
    ...
    def astar(self, map, start_node, end_node):
        ...
        q.put(ComparableObj(0, np.sqrt(np.square(end_node[0]-start_node[0])*np.square(end_node[1]-start_node[1])), start_node))
                ...
                for j in range(0, 4):
                    ...
                    if not map[neighborNode[0]][neighborNode[1]]:
                        ...
                        q.put(ComparableObj(distance_traveled,
                                            np.sqrt(np.square(end_node[0] - neighborNode[0]) * np.square(end_node[1] - neighborNode[1])),
                                            neighborNode))
        print "not distance"
